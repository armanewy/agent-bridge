...
The broker blocks path traversal, sanitizes file names, and enforces that reads stay inside the AgentBridge artifact root. Secret scanning and provider-upload approval are later policy layers; the current broker is local-only.
